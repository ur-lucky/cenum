...
pub fn generate(options: &BuildOptions) -> String {
    ...
    if options.use_basic {
        basic::emit_runtime(&mut output, options);
    } else {
        runtime::emit_runtime(&mut output, options);
    }

    let emitter: Box<dyn SolverEmitter> = if options.use_basic {
        Box::new(basic::BasicEmitter)
    } else {
...
mod tests {
    ...
    #[test]
    fn basic_mode_uses_basic_uenum_runtime_without_serdes() {
        let mut options = options(Solver::Old);
        options.use_basic = true;

        let output = generate(&options);

        assert!(output.contains("const function __CEnumNewBasicEnum<T>(enum: T): T"));
        assert!(output.contains("Duplicate enum value"));
        assert!(output.contains("local TransactionType = __CEnumNewBasicEnum({"));
        assert!(output.contains("\tRobux = 1,"));
        assert!(output.contains("\tTickets = 2,"));
        assert!(output.contains("export type TransactionTypeKey = \"Robux\" | \"Tickets\""));
        assert!(output.contains("export type TransactionType = number"));
        assert!(output.contains("Robux: number,"));
        assert!(output.contains("[number]: TransactionTypeKey,"));
        assert!(output.contains("TransactionStatus = TransactionStatus,"));
        assert!(!output.contains("__CEnumTransactionTypeItemName"));
        assert!(!output.contains("__CEnumSerialize"));
        assert!(!output.contains("__CEnumDeserialize"));
